AT.prototype.atFormHelpers = {
    ...
    showTitle: function(next_state){
        ...
        if (Meteor.userId() && state === "signIn" && !isGuest())
          return false;
    ...
    showOauthServices: function(next_state){
        ...
        if (!services.length)
            return false;
         if (Meteor.userId() && !isGuest())      //CHECK IF USER IS GUEST, IF SO SHOW SERVICES       
    ...
    showServicesSeparator: function(next_state){
        ...
        return rightState && (!Meteor.userId()|| isGuest()) && pwdService && AccountsTemplates.oauthServices().length;
    },
    ...
    showPwdForm: function(next_state) {
        if (Package["accounts-password"] === undefined)
            return false;
        var state = next_state || this.state || AccountsTemplates.getState();
        if ((state === "verifyEmail") || (state === "signIn" && Meteor.userId() && !isGuest()))
            return false;
        return true;
    },
    ...
    showSignUpLink: function(next_state){
        if  (AccountsTemplates.options.hideSignUpLink)
            return false;
        var state = next_state || this.state || AccountsTemplates.getState();
        return ((state === "signIn" && (!Meteor.userId() || isGuest())) || state === "forgotPwd") && !AccountsTemplates.options.forbidClientAccountCreation;
    },
